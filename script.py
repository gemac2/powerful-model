...
def get_klines(tick, interval, limit=48):
    try:
        klines = client.futures_klines(symbol=tick, interval=interval, limit=limit, timeout=30)
        timeframe = interval.replace('KLINE_INTERVAL_', '').replace('_', ' ')
        return klines, timeframe
    except Exception as e:
        print(f"Error getting {timeframe} klines for {tick}: {e}")
        ...
        if klines is not None:
            ...
            if tick not in last_signal_time or (time.time() - last_signal_time[tick]) > wait_time:
                ...
                if close_price <= lower_band.iloc[-1] and rsi <= RSI_OVERSOLD:
                    ...
                    if info and float(info['quoteVolume']) >= IDEAL_VOLUME:
                        signals.append(("⚠️ Powerful Patron", timeframe, "Possible Long", tick, info))
                ...
                # Short signal
                    ...
                    info = get_info_ticks(tick)
                    if info and float(info['quoteVolume']) >= IDEAL_VOLUME:
                        signals.append(("⚠️ Powerful Patron", timeframe, "Possible Short", tick, info))
                        last_signal_time[tick] = time.time()

            # Movement alerts
            movement_klines, _ = get_klines(tick, Client.KLINE_INTERVAL_1MINUTE, 30)
            if movement_klines is not None:
                ...
                if len(movement_klines) >= 3:
                    ...
                    if initial < final and (variation := round(((final - initial) / initial) * 100, 2)) >= FAST_VARIATION_THRESHOLD:
                        info = get_info_ticks(tick)
                        if info:
...
while True:
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for signals in executor.map(process_signals, ticks):
